...
async function getParcelles(departement, cadastreMillesime) {
  try {
...
async function getParcellesDepartement(departement, cadastreMillesime) {
  return chain(await getParcelles(departement, cadastreMillesime))
    .compact()
    .flatten()
    .keyBy('properties.id')
    .value()
